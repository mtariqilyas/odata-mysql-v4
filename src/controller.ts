...
import {connect, formatSql} from "./connect";
...
export class ProductsController extends ODataController {
    ...
    async find( @odata.query query: ODataQuery): Promise<Product[]> {
        ...
        const where = sqlQuery.where.split('?').filter((x, i, a) => a.indexOf(x) == i).join(' ? ');
        const s = sqlQuery.select.split(',')
        const select = s.filter((x, i, a) => a.indexOf(x) == i && x.indexOf('``') == -1).join(', ');
        const sql = `SELECT ${select} FROM Products WHERE ${where}`;
        
        const params = sqlQuery.parameters.filter((x, i, a) => a.indexOf(x) == i);
        
        
        const results = await db.query(formatSql(sql, [...params]));
    ...
    async upsert( @odata.key key: number, @odata.body data: any, @odata.context context: any): Promise<Product> {
        ...
        const params = [key, data.QuantityPerUnit, data.UnitPrice, data.CategoryId, data.Name, data.Discontinued,
